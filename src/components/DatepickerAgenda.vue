    ...
    <div class="datepicker-body" :class="[classOrientation]">
      ...
      <div class="datepicker-month" v-for="month in months">
        <transition-group name="slidev">
          <div class="datepicker-month-label" v-for="month in [month]" :class="classDirection" :key="month" @click="showOrHideMonths">
            {{ month.getFormatted() }}
          </div>
        </transition-group>
      ...
      <div class="datepicker-listMonths" :class="[isDoubled, classOrientation]" v-show="monthsVisible" transition="fade">
        <div class="datepicker-listMonths-content">
          <div class="datepicker-listMonth" v-for="month in listMonths" :key="month.month" @click="selectMonth(month.date)">
            {{ month.name }}
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      this.listMonths = this.months[0].getMonths()
    });
  },
  methods: {
    classYear(year) {
    ...
    showOrHideMonths() {
      this.monthsVisible = !this.monthsVisible
    }
  }
};
</script>
